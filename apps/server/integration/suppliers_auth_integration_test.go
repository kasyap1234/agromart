...
req, _ := http.NewRequest(http.MethodPost, baseURL+"/api/auth/login", strings.NewReader(`{"email":"admin@example.com","password":"password"}`))
...
// simple extraction to avoid tight coupling
s := string(all)
start := strings.Index(s, `"token"`)
require.NotEqual(t, -1, start)
return s // token string presence validated; subsequent calls send raw Authorization for smoke
}
